...
public class MakeGraph {
    ...
    public void bfs(Node root) throws IOException{
        ...
        try (FileWriter myWriter = new FileWriter("D:\\cfgjava\\cfgjava\\CFG\\cfg\\LeveledNodes.txt")) {
    ...
    public void printGraph (){
        ...
        System.out.println("Cyclomatic Complexity : "+(edge-node+1));
    }


    public void saveGraph() throws IOException{
        try (FileWriter myWriter = new FileWriter("D:\\cfgjava\\cfgjava\\CFG\\cfg\\Edges.txt")) {
            //myWriter.write((Lines.size())+"\n");
            for(int i=0; i<Lines.size(); i++){
                for(int j=0; j<Lines.size(); j++){
                    if(adj[i][j]==1){
